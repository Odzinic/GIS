...
elevField = arcpy.GetParameterAsText(4)
upperSTD = arcpy.GetParameter(5)
lowerSTD = arcpy.GetParameter(6)
outputFields = ["POINT_X", "POINT_Y", "Obj_ID", yieldField, "Path_Num"]
...
for key in pointDic.keys():                                                             # Iterate through all of the keys in the point dictionary(pointDic)
        ...
        if (abs(prevDegree - currDegree) > 200):
            
            if (abs(360 - abs(currDegree - prevDegree)) > threshDegree):
        ...
        elif (abs(currDegree - prevDegree) >= threshDegree):                                        # Checks to see if the degree difference is 50 or more
            ...
            try:
                lookahCheck = (abs((pointDic[key + lookaheadConst][0] - currDegree)) < 
...
for path in pathLen.keys():
    locYield = globYield = arcpy.da.TableToNumPyArray(points, [yieldField], '"Path_Num" = {0}'.format(path)) # @UndefinedVariable
...
with arcpy.da.UpdateCursor(points, ["FID", degreeField, elevField, "Path_Num", yieldField]) as cursor: # @UndefinedVariable
    for row in cursor:
...
for field in outputFields:
    ...
    tableFields.addInputField(points, field)
